...
(function($) {
	...
	function headerStyle() {
		if($('.main-header').length){
			...
			var headerLogo = $('.main-header .logo-box .logo img');
			var defaultLogoSrc = headerLogo.data('default-src') || headerLogo.attr('src');
			var stickyLogoSrc = headerLogo.data('sticky-src') || 'images/logo-white.png';
			var scrollLink = $('.scroll-to-top');
			if (windowpos >= 1) {
				siteHeader.addClass('fixed-header');
				if (headerLogo.length) {
					headerLogo.attr('src', stickyLogoSrc);
				}
				scrollLink.fadeIn(300);
			} else {
				siteHeader.removeClass('fixed-header');
				if (headerLogo.length) {
					headerLogo.attr('src', defaultLogoSrc);
				}
